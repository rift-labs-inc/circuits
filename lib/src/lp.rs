...
pub fn encode_liquidity_providers(
    ...
    assert!(liquidity_providers.len() <= MAX_LIQUIDITY_PROVIDERS, "Too many liquidity providers");
    ...
    for i in 0..liquidity_providers.len() {
        ...
        liquidity_providers_encoded[i][2][0..22].copy_from_slice(&liquidity_providers[i].script_pub_key);
    }
